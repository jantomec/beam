...
class TestSimoElement(unittest.TestCase):
    ...
    def test_static_cantilever(self):
        correct = np.zeros(3)
        system = test_static_cantilever.cantilever(printing=False)
        system.solve()
        u = system.coordinates[:,-1] + system.displacement[-1][:,-1]
    ...
    def test_static_cantilever_displacement_control(self):
        ...
        system = test_static_cantilever_displacement_control.cantilever(printing=False)
        system.solve()
        u = system.displacement[-1][:,1]
        self.assertTrue(np.allclose(u, correct, rtol=1e-10))
    ...
    #     u = test_dynamic_frame.frame(t_end=30.0, printing=False)[-1]
    #     self.assertTrue(np.allclose(u, correct, rtol=1e-10))
