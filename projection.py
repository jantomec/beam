...
def circular_point_projection(
    ...
    x2a = (X2 @ intp.lagrange_poly(n_nodes_2-1, [s1+0.01])).flatten()
    x2b = (X2 @ intp.lagrange_poly(n_nodes_2-1, [s1-0.01])).flatten()
    u = np.zeros((5))
    u[0] = s1
    u[1:3] = (x1 - x2b) / 2
    u[3:5] = (x2a - x1) / 2
    ...
    while np.linalg.norm(R) > TOLER and i < MAXITER:
        ...
        if interpolation == "Lagrange polynoms":
            ...
            dx2 = (X2 @ intp.lagrange_poly_d(n_nodes_2-1, [s2])).flatten()
            ddx2 = (X2 @ intp.lagrange_poly_dd(n_nodes_2-1, [s2])).flatten()
            R[:2] = x1 - (x2 + v2 - v1)
...
def _spherical_point_projection(
    ...
    while  abs(R) > TOLER and n < MAXITER:
        ...
        if interpolation == "Lagrange polynoms":
            ...
            dx2 = (X2 @ intp.lagrange_poly_d(n_nodes_2-1, [s2])).flatten()
            ddx2 = (X2 @ intp.lagrange_poly_dd(n_nodes_2-1, [s2])).flatten()
            dddx2 = (X2 @ intp.lagrange_poly_dd(n_nodes_2-1, [s2])).flatten()
            Ai = A_(x1, x2, dx1, dx2)
